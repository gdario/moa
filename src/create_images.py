...
datasets_trt = [utils.create_fname(datasets_trt[i]) for i in
                range(len(datasets_trt))]
...
if SAVE_IMAGES:
    for i in range(len(image_arrays)):
        ...
        for j in range(image_arrays[i].shape[0]):
            utils.save_image(image_arrays[i][j],
                             datasets_trt[i].fname.iloc[j],
                             img_folders[i])
...
datasets_trt[0][['fname', 'labels', 'is_valid']].to_csv(
